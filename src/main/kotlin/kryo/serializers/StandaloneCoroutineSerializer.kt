...
internal class StandaloneCoroutineSerializer(kryo: Kryo, type: Class<*>): FieldSerializer<Any>(kryo, type) {
    private val _parentContext = fields.first { it.field.declaringClass == type && it.field.name == "parentContext" }.also { removeField(it) }

    override fun create(kryo: Kryo?, input: Input?, type: Class<out Any>?): Any? {
